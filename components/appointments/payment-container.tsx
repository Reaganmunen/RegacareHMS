...
const columns = [
  { header: "No", key: "id" },
  { header: "Bill Date", key: "bill_date", className: "" },
  { header: "Payment Date", key: "payment_date", className: "hidden md:table-cell" },
  { header: "Total", key: "total_amount", className: "" },
  { header: "Discount", key: "discount", className: "hidden xl:table-cell" },
  { header: "Paid", key: "payable", className: "hidden xl:table-cell" },
...
export const PaymentsContainer = async ({ patientId }: { patientId: string }) => {
  ...
  const renderRow = (item: Payment) => {
    return (
      ...
      >
        <td className="flex items-center gap-2 md:gap-4 py-2 xl:py-4">#{item.id}</td>
        <td className="lowercase">{format(new Date(item.bill_date), "MMM d, yyyy")}</td>
        <td className="hidden items-center py-2 md:table-cell">
          {item.payment_date ? format(new Date(item.payment_date), "MMM d, yyyy") : "-"}
        </td>
        <td>{item.total_amount.toFixed(2)}</td>
        <td className="hidden xl:table-cell">{item.discount?.toFixed(2) ?? "0.00"}</td>
        <td className="hidden xl:table-cell">{(item.total_amount - (item.discount ?? 0)).toFixed(2)}</td>

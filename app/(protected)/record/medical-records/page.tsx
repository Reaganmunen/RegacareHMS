...
const renderRow=(item: ExtendedProps)=>{
  ...
  className="border-b border-gray-200 even:bg-slate-50 text-base hover:bg-slate-50">
    <td className="flex items-center gap-4 p-4">
      <ProfileImage url={item?.patient?.img!}
      name={name}  colorCode={patient?.colorCode!}
    ...
    <td className="hidden md:table-cell">{
        format(item?.created_at, "yyyy-MM-dd HH:mm:ss")}</td>
    <td className="hidden 2xl:table-cell">{item?.doctor_id}</td>
    <td className="hidden lg:table-cell">{item?.diagnosis?.length===0
      ? <span className="text-gray-400 italic">No Diagnosis Found</span>:<span>{item?.diagnosis.length}</span>
        
        }</td>
    <td className="hidden xl:table-cell">{item?.lab_test?.length===0
      ? <span className="text-gray-400 italic">No Labtest Found</span>:<span>{item?.lab_test.length}</span>

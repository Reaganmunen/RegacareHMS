...
const renderRow=(item: ExtendedProps)=>{
  ...
  className="border-b border-gray-200 even:bg-slate-50 text-base hover:bg-slate-50">
    ...
    <td className="hidden md:table-cell">{patient?.phone_number}</td>
    <td className="hidden md:table-cell">{
        format(item?.bill_date, "yyyy-MM-dd")}</td>
    ...
    <td className="hidden xl:table-cell">{(item?.total_amount - item?.discount).toFixed(2)}</td>
     ...
     <td><span className={cn(item?.payment_status==="Unpaid" ? "text-red-600": 
        item?.payment_status==="Paid" ?"text-emerald-600" : "text-gray-600"

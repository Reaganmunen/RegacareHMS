...
interface PatientProps extends Patient {
  ...
  appointments:{
    medical:{
          created_at:Date, treatment_plan:string
    }[];
  }[];
}
...
const renderRow=(item: PatientProps)=>{
  ...
  className="border-b border-gray-200 even:bg-slate-50 text-base hover:bg-slate-50">
    <td className="flex items-center gap-4 p-4">
      ...
      name={name}  colorCode={item?.colorCode!}
    ...
    <td className="hidden xl:table-cell">{
      ...
      lastVisit ? format(lastVisit?.created_at, "yyyy-MM-dd HH:mm:ss"):(
      ...
      lastVisit ? (lastVisit?.treatment_plan):(
